...
1. [项目简介](#-项目简介)
2. [🌊🦞 一键装入 OpenClaw](#-一键装入-openclaw)
3. [设计理念](#-设计理念)
4. [核心概念：CAS 如何驱动预测](#-核心概念cas-如何驱动预测)
5. [星海合议架构](#-星海合议架构)
6. [运行时引擎](#-运行时引擎)
7. [系统架构](#-系统架构)
8. [快速开始](#-快速开始)
9. [成本对比](#-成本对比)
10. [社交媒体：第一个领域实现](#-社交媒体第一个领域实现)
11. [PMF 验证：第二个领域实现](#-pmf-验证第二个领域实现)
12. [无限可能](#-无限可能)
13. [工程结构](#-工程结构)
14. [项目状态](#-项目状态)
15. [技术栈](#-技术栈)
16. [文档索引](#-文档索引)
17. [灵感来源：OASIS](#-灵感来源oasis)
18. [致谢](#-致谢)
19. [许可证](#-许可证)
...
适用于 macOS / Linux / Windows (WSL)。脚本会：
...
- 将任务数据库与产物目录固定到 `~/.ripple/data/` 下；源码开发态仍默认使用当前目录下的 `./data/` 与 `./ripple_outputs/`
...
├── cli/                    # 🖥️ 命令行入口
│   └── app.py              #   ripple-cli — LLM / domain / job 子命令
├── service/                # 🌐 HTTP+SSE 服务
│   ├── app.py              #   FastAPI 应用与接口定义
│   ├── auth.py             #   Bearer Token 鉴权
│   ├── event_bus.py        #   任务事件总线（SSE）
│   ├── job_manager.py      #   任务编排与生命周期管理
│   ├── job_repo_sqlite.py  #   SQLite 任务仓库
│   ├── reporting.py        #   服务侧产物/报告读取
│   ├── runner.py           #   后台任务执行器
│   └── settings.py         #   服务配置
├── utils/                  # 🔧 工具函数
│   └── json_parser.py      #   JSON 解析辅助
├── reporting.py            # 📄 模拟后解读报告生成
├── version.py              # 🏷️ 项目版本读取
...
│   ├── request-schema.yaml #   请求输入 Schema
│   ├── platforms/           #   7 个平台画像文件
│   │   ├── xiaohongshu.md  #     🔴 小红书
│   │   ├── douyin.md       #     🎵 抖音
│   │   ├── weibo.md        #     🔥 微博
...
├── e2e_ab_test_fmcg_coffee.py              # PMF 多方案对比示例
├── e2e_simulation_xiaohongshu.py           # 小红书完整模拟示例
├── e2e_simulation_cny_robot_xiaohongshu.py # 小红书 CNY 场景模拟
├── e2e_simulation_xiaohongshu_service.py   # 小红书 HTTP+SSE 服务调用示例
